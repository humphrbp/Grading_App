...
//Code to Get Input for All Scores//

		Scanner scan = new Scanner(System.in);

		int gradecol = 8; /* 8 Grades per student */
		...
		int[][] scores = new int[gradecol][students];

		// Get Grades from User Input

		for (int j = 0; j < students; j++) {

			EnterStudentScores(scan, scores, students, gradecol, j);

		}

		scan.close();

		// Sum all grades for each student

		int score_sum[] = new int[students];

		for (int l = 0; l < students; l++) {

			int sum = 0;

			for (int i = 0; i < gradecol; i++) {

				sum = sum + scores[i][l]; // Adds all assignments + quizes
